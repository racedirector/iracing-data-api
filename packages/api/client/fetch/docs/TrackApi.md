...
|------------- | ------------- | -------------|
| [**getTrack**](TrackApi.md#gettrack) | **GET** /data/track/get |  |
| [**getTrackAssets**](TrackApi.md#gettrackassets) | **GET** /data/track/assets |  |
...
import type { GetTrackRequest } from '@iracing-data/api-client-fetch';
...
import type { GetTrackAssetsRequest } from '@iracing-data/api-client-fetch';
...
async function example() {
  ...
  try {
    const data = await api.getTrackAssets();
    console.log(data);
  } catch (error) {
    console.error(error);
  }
